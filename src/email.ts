...
const wrap = (title: string, bodyHtml: string) => `
  <div style="font-family:Georgia,serif;background:#FAF7F0;padding:32px 16px;">
    <div style="max-width:560px;margin:0 auto;background:#ffffff;border-radius:16px;overflow:hidden;border:1px solid #EAE1D0;">
      <div style="background:#103C26;color:#FAF7F0;padding:24px 28px;">
        <div style="font-size:11px;letter-spacing:2px;text-transform:uppercase;color:#E8C86A;font-weight:bold;">Dadi Industries</div>
        <h1 style="margin:6px 0 0;font-size:20px;">${title}</h1>
      </div>
      <div style="padding:24px 28px;color:#103C26;">${bodyHtml}</div>
...
export const sendOrderConfirmationEmail = async (order: OrderForEmail): Promise<void> => {
  ...
  try {
    await resend.emails.send({
      ...
      subject: `Order Confirmed — ${order.orderId} | Dadi Industries`,
      html
    });
  } catch (error) {
...
export const sendWelcomeEmail = async (to: string, name: string): Promise<void> => {
  ...
  const html = wrap(
    'Welcome to Dadi Industries',
    `<p>Namaste ${name},</p><p>Your Patron account is ready. Explore our handcrafted pickles, chutneys and murabbas, and enjoy faster checkout and live order tracking.</p>`
  );
  try {
    await resend.emails.send({
      from: config.resendFromEmail,
      to,
      subject: 'Welcome to Dadi Industries',
      html
    });
  } catch (error) {
...
export const sendVerificationEmail = async (to: string, name: string, code: string): Promise<void> => {
  ...
  const html = wrap(
    'Verify your Dadi Industries account',
    `<p>Namaste ${name},</p><p>Use this one-time verification code to finish creating your account:</p><p style="font-size:32px;letter-spacing:8px;font-weight:bold;text-align:center;color:#103C26;margin:24px 0;">${code}</p><p>This code expires in 10 minutes. If you did not request an account, you can ignore this email.</p>`
  );
  await resend.emails.send({
    from: config.resendFromEmail,
    to,
    subject: 'Verify your Dadi Industries account',
    html
  });
};
...
export const sendPasswordResetEmail = async (to: string, name: string, code: string): Promise<void> => {
  ...
  const html = wrap(
    'Reset your Dadi Industries password',
    `<p>Namaste ${name},</p><p>Use this one-time code to reset your Dadi Industries password:</p><p style="font-size:32px;letter-spacing:8px;font-weight:bold;text-align:center;color:#103C26;margin:24px 0;">${code}</p><p>This code expires in 10 minutes. If you did not request a password reset, you can ignore this email.</p>`
  );
  await resend.emails.send({
    from: config.resendFromEmail,
    to,
    subject: 'Reset your Dadi Industries password',
    html
  });
};
